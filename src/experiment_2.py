...
with torch.no_grad():
    logits_yes = model(
        input_ids_yes, decoder_input_ids=decoder_input_ids_yes
    ).logits[:, -1, :]
    logits_no = model(
        input_ids_no, decoder_input_ids=decoder_input_ids_no
    ).logits[:, -1, :]

# Calculate the likelihood of "yes" and "no"
likelihood_yes = torch.exp(
    logits_yes[:, tokenizer.convert_tokens_to_ids("yes")]
).item()
likelihood_no = torch.exp(
    logits_no[:, tokenizer.convert_tokens_to_ids("no")]
).item()
