...
def generate_predictions(
   ...
   # Generate the (missing) predictions.
    ...
    for i, (example, full_prompt) in enumerate(
        ...
        first_prediction_response = pipe(
                full_prompt,
        ...
        first_prediction = first_prediction_response[0]["generated_text"]
        if isinstance(pipe, transformers.pipelines.TextGenerationPipeline):
            first_prediction = first_prediction[len(full_prompt) :].lstrip()

        # ================ Debugging ================
        logging.debug(full_prompt)
        logging.debug("===================prompt==================")
        logging.debug("====================pred===================")
        logging.debug(first_prediction)
        logging.debug("=================2nd prompt================")
        # ================ Debugging ================
        ...
        if chain_of_thought:
            ...
            # Extract the model's output prediction from the second response.
            final_prediction = second_prediction_response[0]["generated_text"]
            if isinstance(pipe, transformers.pipelines.TextGenerationPipeline):
                final_prediction = final_prediction[len(second_prompt) :].lstrip()

            # ================ Debugging ================
            logging.debug("====================final===================")
            logging.debug(final_prediction)
            logging.debug("================correct answer==============")
            logging.debug(example["offensiveYN"])
            logging.debug("============================================")
            # ================ Debugging ================
