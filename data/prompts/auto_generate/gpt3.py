...
# Get the API key from the corresponding environment variable.
...
def get_completion(
    ...
    Args:
        ...
        temperature (int, optional): What sampling temperature to use, between
            0 and 2. Higher values like 0.8 will make the output more random,
            while lower values like 0.2 will make it more focused and
            deterministic.
        max_tokens (int, optional): Thee maximum number of tokens to generate
            in the completion. The token count of your prompt plus max_tokens
        ...
        top_p (float, optional): An alternative to sampling with temperature,
            ...
            the tokens with top_p probability mass. So 0.1 means only the
...
def get_chat_completion(
    ...
    Args:
        messages (list[dict[str, str]]): The user and system messages to
            complete.
        model (str, optional): The model to use. Defaults to
            settings.OPENAI_MODEL.
        temperature (int, optional): What sampling temperature to use, between
